...
void JulyHttp::readSocket()
{
	...
	if(allDataReaded)
	{
        ...
        {
            QByteArray cdata(compressBound(buffer.length()), '\0');
            uLongf len = compressBound(buffer.length());

            if(contentGzipped)uncompress(reinterpret_cast<unsigned char*>(cdata.data()), &len, reinterpret_cast<unsigned char*>(buffer.data()), buffer.length());
